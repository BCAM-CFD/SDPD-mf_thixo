...
//ASE add the calculation of the gamma_dot
        ...
        /*
        gammadot[i] = sqrt(2*(
        gradv[i][0] * gradv[i][0] +  // dvx/dx²
        gradv[i][1] * gradv[i][1] +  // dvy/dy²
        gradv[i][2] * gradv[i][2] +  // dvz/dz²
        2*((0.5*(gradv[i][3] + gradv[i][6])) * (0.5*(gradv[i][3] + gradv[i][6])) +  // (dvx/dy + dvy/dx)²
        (0.5*(gradv[i][4] + gradv[i][7])) * (0.5*(gradv[i][4] + gradv[i][7])) +  // (dvx/dz + dvz/dx)²
        (0.5*(gradv[i][5] + gradv[i][8])) * (0.5*(gradv[i][5] + gradv[i][8])))    // (dvy/dz + dvz/dy)²
        ));
	*/
	
	gammadot_tmp = gammadot[i];  //*(1-wall_index[itype][jtype]) + g_d*wall_index[itype][jtype];            
                if (gammadot_tmp == 0) {
...
/*
        ...
        f_a = (1.0/(1.0+beta_thixo*1))*(1.0 - exp_thixo);
