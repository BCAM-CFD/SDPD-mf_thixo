...
/* ---------------------------------------------------------------------
  ...
  Modified version of SDPD including explicitly bulk viscosity as presented
	by Espanol and Revenga, including background pressure, and generalizing for 2 and 3 dimensions.
	
  update non-slip in flat wall: This update included the option for non-slip boundary condition in a flat
  ...
  Update MF: This updated multiphase (mf) version is based on the description presented in
...
//ASE add the calculation of the gamma_dot

	//gammadot_tmp = gammadot[i];

	//gammadot_tmp = gammadot[i];//*(1-wall_index[itype][jtype]) + g_d*wall_index[itype][jtype];                 //to avoid inestabilities and per 0 division
                //if (gammadot_tmp == 0) {
                //        gammadot_tmp = 1e-10;           //Condition to avoid dividing by 0
                //}
        
        //gammadot[i] = gammadot[i]*(1-wall_index[itype][jtype]) + g_d*wall_index[itype][jtype];                 //to avoid inestabilities and per 0 division
...
//Boolen operator to compute viscosity (0 if i=j and 1 if i =/ j)
	...
	//double g_dd = gradv[i][3];
	
	//double gamma = g_dd*(1-wall_index[itype][jtype]) + g_d*wall_index[itype][jtype];
//std::cout << "gradv[i][3][" << j << "] = " << g_d << std::endl;
...
// fluid_type==2 for Thixotropic model
  		...
  		beta_thixo = (b_thixo[itype][jtype]/a_thixo[itype][jtype]);
        	...
        	exp_thixo = exp(-(1+beta_thixo*gammadot[i])*curTime/lambda_time_scale);

        	f_a = (1.0/(1.0+beta_thixo*gammadot[i]))*(1.0 - exp_thixo);
...
/*
        beta_thixo = (b_thixo[itype][jtype]/a_thixo[itype][jtype])*(1-wall_index[itype][jtype]) + (b_thixo[jtype][jtype]/a_thixo[jtype][jtype])*wall_index[itype][jtype];
        lambda_time_scale = (1.0/a_thixo[itype][jtype])*(1-wall_index[itype][jtype])+(1.0/a_thixo[jtype][jtype])*wall_index[itype][jtype];
        exp_thixo = exp(-(1+beta_thixo*gammadot[i])*curTime/lambda_time_scale);

        f_a = (1.0/(1.0+beta_thixo*gammadot[i]))*(1.0 - exp_thixo);
        f_b = (f0_thixo[itype][jtype]*exp_thixo)*(1-wall_index[itype][jtype]) + (f0_thixo[jtype][jtype]*exp_thixo)*wall_index[itype][jtype];
        f_t = f_a + f_b;

        //eta_12= (2*((viscosity[itype][jtype]*(1.0+alpha_thixo[itype][jtype]*f_t))*(viscosity[itype][jtype]*(1.0+alpha_thixo[itype][jtype]*f_t))) / (((viscosity[itype][jtype]*(1.0+alpha_thixo[itype][jtype]*f_t)))+((viscosity[itype][jtype]*(1.0+alpha_thixo[itype][jtype]*f_t)))))*(1-wall_index[itype][jtype]) + (viscosity[jtype][jtype]*(1.0+alpha_thixo[jtype][jtype]*f_t))*wall_index[itype][jtype];

        //eta_12= sqrt((viscosity[itype][jtype]*(1.0+alpha_thixo[itype][jtype]*f_t))*(viscosity[itype][jtype]*(1.0+alpha_thixo[itype][jtype]*f_t)))*(1-wall_index[itype][jtype]) + (viscosity[jtype][jtype$

        eta_12= (2*(viscosity[itype][jtype]*viscosity[itype][jtype]) / (viscosity[itype][jtype] + viscosity[itype][jtype]))*(1-wall_index[itype][jtype]) + viscosity[jtype][jtype]*wall_index[itype][jtype];

        bulk_eta_12 = 0.999*eta_12*(2.0*dim-1.0)/dim;

        visco = (viscosity[itype][jtype]*(1.0+alpha_thixo[itype][jtype]*f_t))*(1.0-f12) + eta_12*(1.0+(alpha_thixo[itype][jtype]*(1-wall_index[itype][jtype]) + alpha_thixo[jtype][jtype]*wall_index[itype][jtype] )*f_t)*(f12);
        bulk_visco = (0.999*visco*(2.0*dim-1.0)/dim)*(1.0-f12) + bulk_eta_12*(f12);
*/

// fluid_type==3 for DST model
        } else if (fluid_type==3){
	visco = viscosity[itype][jtype];
	bulk_visco = 0.999*visco*(2.0*dim-1.0)/dim;               
        }	
...
void PairSDPDTaitwaterIsothermalMf::allocate () {
  ...
  memory->create (tau_thixo, n+1, n + 1, "pair:tau_thixo");
  memory->create (m_thixo, n+1, n + 1, "pair:m_thixo");      
...
void PairSDPDTaitwaterIsothermalMf::settings (int narg, char **arg) {
  ...
  /* ASE adding factors g_d nad fluid_type as variables */
  g_d = utils::numeric(FLERR, arg[7],false,lmp);
  fluid_type = utils::numeric(FLERR, arg[8],false,lmp);
  ...
  /* ASE adding conditional for model number F. It had to be an integer*/
  if (fluid_type < 1 || fluid_type > 3) error->all(FLERR, "Viscosity model must be an integer between 1 and 3");   


  // seed is immune to underflow/overflow because it is unsigned
  seed = comm->nprocs + comm->me + atom->nlocal;
  if (narg == 10) seed += utils::inumeric(FLERR, arg[9], false, lmp);
#ifdef USE_ZEST
  generator.seed (seed);
#else
...
void PairSDPDTaitwaterIsothermalMf::coeff (int narg, char **arg) {
  ...
  double tau_thixo_one = utils::numeric(FLERR,arg[12], false, lmp);
  double m_thixo_one = utils::numeric(FLERR,arg[13], false, lmp);
  ...
  double S_one = utils::numeric(FLERR,arg[14], false, lmp); //Default no interaction between same species 
  double sb_one = utils::numeric(FLERR,arg[15], false, lmp);

  if (narg==18){
  	///ADDITIONAL ERROR CATCHING LINES NEEDED TO ACCOUNT FOR PROPER POSITION OF THE WALL
    slip_one = utils::numeric(FLERR,arg[16], false, lmp);
    bccoord_one = utils::numeric(FLERR,arg[17],false,lmp);
   }
  ...
  for (int i = ilo; i <= ihi; i++) {
    ...
    for (int j = MAX(jlo,i); j <= jhi; j++) {
      ...
      tau_thixo[i][j]=tau_thixo_one;
      m_thixo[i][j]=m_thixo_one;
      S[i][j] = S_one;
      sb[i][j] = sb_one;
      setflag[i][j] = 1;
      count++;
...
double PairSDPDTaitwaterIsothermalMf::init_one (int i, int j) {
  ...
  tau_thixo[j][i] = tau_thixo[i][j];
  m_thixo[j][i] = m_thixo[i][j];
